...
var renderizar = function(year, regiaoId, version, formatoE, modeloE, backboneE, formatoD, modeloD, backboneD) {
  ...
  var getPred = function(modelo, backbone, fmt) {
    ...
    // npy usa neq para capturar assets sem propriedade definida
    col = fmt === 'npy'
      ? col.filter(ee.Filter.neq('formato', 'tfr'))
      : col.filter(ee.Filter.eq('formato', 'tfr'));
    print("quero ver formato ", fmt );
    print("saber o numero de imagens antes de version ", col.size());
    col = col.filter(ee.Filter.eq('version', version));
